...
class QDMCControllerConfiguration:
    ...
    def __init__(
        ...
        if policy == "strict" and (rho_min is not None or rho_max is not None):
            raise ValueError(
                f'Parameters rho_min and rho_max are invalid for policy "{policy}"'
            )
        if policy in ("soft", "minimize_violation") and (
            rho_min is None or rho_max is None
        ):
            raise ValueError(
                f'Parameters rho_min and rho_max are required for policy "{policy}"'
            )
...
class QDMCControllerState(BaseDMCState):
    ...
    class _ProblemData:
        ...
        def __init__(self, config: QDMCControllerConfiguration):
            ...
            du_min_rep_np = (
                np.tile(config.du_min.cpu().numpy(), config.Nu)
                if config.du_min is not None
                else None
            )
            du_max_rep_np = (
                np.tile(config.du_max.cpu().numpy(), config.Nu)
                if config.du_max is not None
                else None
            )
            u_min_rep_np = (
                np.tile(config.u_min.cpu().numpy(), config.N)
                if config.u_min is not None
                else None
            )
            u_max_rep_np = (
                np.tile(config.u_max.cpu().numpy(), config.N)
                if config.u_max is not None
                else None
            )
            y_min_rep_np = (
                np.tile(config.y_min.cpu().numpy(), config.N)
                if config.y_min is not None
                else None
            )
            y_max_rep_np = (
                np.tile(config.y_max.cpu().numpy(), config.N)
            ...
            if y_min_rep_np is not None:
                ...
                soft_constraints.append(
                    y_min_rep_np - y0_rep_np - self.epsilon_min
                    <= self.free_traj_param + M_np @ self.x
                )
            if y_max_rep_np is not None:
                ...
                soft_constraints.append(
                    y_max_rep_np - y0_rep_np + self.epsilon_max
                    >= self.free_traj_param + M_np @ self.x
                )

            base_objective = cp.sum_squares(
                M_np @ self.x - self.target_param
            ...
            if config.policy == "strict":
                self.problem = cp.Problem(
                    cp.Minimize(base_objective), strict_constraints
                )
                ...
                fallback_objective = config.rho_min * cp.sum_squares(
                    self.epsilon_min
                ) + config.rho_max * cp.sum_squares(self.epsilon_max)
                self.fallback_problem = cp.Problem(
                    cp.Minimize(fallback_objective), soft_constraints
                )
    ...
    def __init__(
        ...
        self.warm_start_x = validate_and_move_optional_tensor(
            warm_start_x,
            "warm_start_x",
            (config.Nu * config.n_inputs,),
            desired_dtype=config.dtype,
            desired_device=config.device,
        )
    ...
    def initial_state_for(
        ...
        past_du, current_u = zero_past_du_and_current_u(
            ...
            controller_configuration.dtype,
            controller_configuration.device,
        )
...
class QDMCControllerClosedSystem(BaseDMCClosedSystem):
    ...
    def step(self, y: torch.Tensor, r_traj: torch.Tensor) -> torch.Tensor:
        ...
        self.state._problem_data.free_traj_param.value = (
            (y_free_flat - y0_rep).cpu().numpy()
        )
        ...
        if (
            self.state._problem_data.x.value is None
            and self.state._problem_data.fallback_problem is not None
        ):
            self.state._problem_data.fallback_problem.solve(
        ...
        if self.state._problem_data.x.value is None:
            if self.config.policy == "strict":
                raise RuntimeError(
                    'QP solver failed in QDMC step. There is no feasible plant input. The current QDMC policy is "strict". Try loosening policy or constraints.'
                )
            else:
                raise RuntimeError(
                    "QP solver failed in QDMC step. There is no feasible plant input. Try loosening constraints."
                )

        du_flat = torch.tensor(
            self.state._problem_data.x.value, dtype=self.dtype, device=self.device
        )
        du = du_flat[: self.plant.n_inputs]

        # Store shifted present step solution as the warm start for next step
        shifted = torch.cat(
            [du_flat[self.plant.n_inputs :], du_flat[: self.plant.n_inputs]], dim=0
        )
        ...
        self.state.past_du = torch.cat(
            (du_actual.unsqueeze(dim=0), self.state.past_du[:-1]), dim=0
        )
