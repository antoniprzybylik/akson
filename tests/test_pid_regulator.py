...
def test_pid_config_rejects_channel_with_nonexisting_input():
    ...
    with pytest.raises(ValueError, match="nonexisting input"):
        PIDRegulatorConfiguration(n_inputs=1, n_outputs=1, channels=[channel], t=0.1)
...
def test_pid_config_rejects_duplicate_channel():
    ...
    with pytest.raises(ValueError, match="defined twice"):
        PIDRegulatorConfiguration(n_inputs=1, n_outputs=1, channels=[channel1, channel2], t=0.1)


def test_pid_config_accepts_multiple_distinct_channels():
    channel1 = PIDChannel(output_idx=0, input_idx=0, K=1.0, Ti=1.0, Td=0.0)
    channel2 = PIDChannel(output_idx=1, input_idx=1, K=-1.0, Ti=2.0, Td=0.0)
    config = PIDRegulatorConfiguration(n_inputs=2, n_outputs=2, channels=[channel1, channel2], t=0.1)
...
def test_pid_config_default_u0_is_none():
    channel = PIDChannel(output_idx=0, input_idx=0, K=1.0, Ti=1.0, Td=0.0)
    config = PIDRegulatorConfiguration(n_inputs=1, n_outputs=1, channels=[channel], t=0.1)
...
def test_pid_config_coefficients_formula():
    ...
    config = PIDRegulatorConfiguration(n_inputs=1, n_outputs=1, channels=[channel], t=t)
...
def test_pid_config_coefficients_are_zero_for_unused_channel_pairs():
    channel = PIDChannel(output_idx=0, input_idx=0, K=1.0, Ti=1.0, Td=0.0)
    config = PIDRegulatorConfiguration(n_inputs=2, n_outputs=2, channels=[channel], t=0.1)
...
def test_pid_state_defaults_are_zero():
    state = PIDRegulatorState(n_inputs=2, n_outputs=3)
...
def test_pid_state_initial_state_for_matches_configuration():
    channel = PIDChannel(output_idx=0, input_idx=0, K=1.0, Ti=1.0, Td=0.0)
    config = PIDRegulatorConfiguration(n_inputs=2, n_outputs=3, channels=[channel], t=0.1)
...
def test_pid_closed_system_rejects_mismatched_n_inputs():
    channel = PIDChannel(output_idx=0, input_idx=0, K=1.0, Ti=1.0, Td=0.0)
    config = PIDRegulatorConfiguration(n_inputs=2, n_outputs=1, channels=[channel], t=0.1)
    state = PIDRegulatorState.initial_state_for(config)
...
def test_pid_closed_system_rejects_mismatched_n_outputs():
    channel = PIDChannel(output_idx=0, input_idx=0, K=1.0, Ti=1.0, Td=0.0)
    config = PIDRegulatorConfiguration(n_inputs=1, n_outputs=2, channels=[channel], t=0.1)
    state = PIDRegulatorState.initial_state_for(config)
...
def test_pid_simulate_returns_consistent_shapes():
    ...
    # 10 regulator steps -> 11 control inputs recorded (including the initial one)
