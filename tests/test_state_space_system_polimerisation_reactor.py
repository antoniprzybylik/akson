...
class FModule(nn.Module):
    ...
    def forward(self, t, x, u):
        ...
        term_prop_tfm = (self.Z_P * math.exp(-self.E_P / (self.R * self.T))) + (self.Z_f_m * math.exp(-self.E_f_m / (self.R * self.T)))
        
        p0_denominator = (self.Z_T_d * math.exp(-self.E_T_d / (self.R * self.T))) + (self.Z_T_c * math.exp(-self.E_T_c / (self.R * self.T)))
        p0_numerator = 2 * self.f_star * torch.clamp(C_I, min=self.eps) * term_init
        P_0 = torch.sqrt(p0_numerator / (p0_denominator + self.eps))
        
        dC_I_dt = -term_init * C_I - (self.F / self.V) * C_I + (F_I * self.C_I_in / self.V)
        dC_m_dt = -term_prop_tfm * C_m * P_0 - (self.F / self.V) * C_m + (self.F * self.C_m_in / self.V)
        
        term_term_d0 = (0.5 * self.Z_T_c * math.exp(-self.E_T_c / (self.R * self.T))) + (self.Z_T_d * math.exp(-self.E_T_d / (self.R * self.T)))
        term_tfm_d0 = self.Z_f_m * math.exp(-self.E_f_m / (self.R * self.T))
        dD_0_dt = term_term_d0 * P_0**2 + term_tfm_d0 * C_m * P_0 - (self.F / self.V) * D_0
...
def test_operating_point_from_input_reactor(reactor_dynamics):
    FI0 = 1.678300e-02
    op = OperatingPoint.from_input(reactor_dynamics, torch.tensor([FI0], dtype=torch.float64))
